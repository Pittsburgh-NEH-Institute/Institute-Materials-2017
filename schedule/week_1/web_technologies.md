...
1. Start \<oXygen/\>.
2. Open [schedule/week_1/web/web_exercise_1.html](web/web_exercise_1.html) in \<oXygen/\>.
...
* CSS instructions consist of a *selector* (which identifies what type of element will be styled) followed by a set of rules enclosed in curly braces. Each rule consist of a *property* and a *value*, separated by a colon, and multiple rules are separated by semicolons. For example, `p {color: red; font-weight: bold;}` embolds all `<p>` elements (since the selector value is `p`) in the document and colors them (since the property name is `color`) red (since the property value is “red”). Selectors can be fine-tuned to target elements only in specific contexts or with specific attributes; we’ll see a bit of that below.

### Looking at CSS

1. Open [schedule/week_1/web/web_exercise_2.html](web/web_exercise_2.html) in \<oXygen/\>.
...
4. Open [schedule/week_1/web/web_exercise_2.css](web/web_exercise_2.css) in \<oXygen/\> and notice that it implements the commands described above, which are responsible for the color and weight of the paragraphs.
...
7. You can set a `background-color` property for any element, or for the entire page (by creating a CSS rule with a selector for “body”, since the `<body>` element contains everything on the page).
...
1. Open [schedule/week_1/web/web_exercise_3.html](web/web_exercise_3.html) in \<oXygen/\>.
...
4. Open [schedule/week_1/web/web_exercise_3.css](web/web_exercise_3.css) in \<oXygen/\>. A CSS selector that begins with a hash mark matches the element in the document that has the text after the hash mark as an `@id` value. We use that feature to color the buttons differently. 
5. CSS supports “red” and “green” as colors, but it’s difficult to read black text against them because they’re dark, so we’ve chosen lighter alternatives. Change the background colors to “red” and “green”, reload the page in the browser, and notice the difference. We could, alternatively, have kept real red and green as backgrounds and changed the value of the `color` property (which controls the color of the text) from the default “black” to something easier to read against a dark background.
...
The `init()` function fires automatically, thanks to the bootstrap line, as soon as the page has finished loading. It finds all of the `<button>` elements (there are two of them), loops over them, and tells them both to listen for click events, that is, to do something when they are clicked. The something they are told to do is to fire the `toggleColor()` function. This function also fires only once.
...
This use of JavaScript to modify the CSS of a page that has already loaded is sometimes called DHTML (dynamic HTML). When the browser renders the HTML, it converts the character-by-character serialized HTML file into a tree in memory (HTML, like XML, is a tree), and what it renders in the browser window is the tree, decorated with CSS. JavaScript works by rewriting the tree in memory; the original HTML and CSS files don’t change, but the in-memory tree does.
